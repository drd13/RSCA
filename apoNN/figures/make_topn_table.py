...
def get_average_n(Z,Z_occam,valid_idxs,N=7):
    """
    Returns an array containing the doppelganger rates of the topN fitters
    Parameters:
        N: number of dimensions to loop over for topN fitter
    """
    averages = []
    for n in range(1,N):  
...
for n in range(num_models):
    ...
    per_sim_averages.append(get_average_n(Z,Z_occam,valid_idxs))
...
for idx,i in enumerate(range(len(averages))):
    print(f"{idx+1} & ${averages[i]:.4f} \pm {averages_err[i]:.4f}$ \\\\")
